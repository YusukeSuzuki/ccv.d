...
const double CCV_PI = 3.141592653589793;
...
T CCV_GET_DATA_TYPE(T)(T x)
{
	return x & 0xFF000;
}

T CCV_GET_DATA_TYPE_SIZE(T)(T x)
{
	return _ccv_get_data_type_size[CCV_GET_DATA_TYPE(x) >> 12];
}

const int CCV_MAX_CHANNEL = (0xFFF);

T CCV_GET_CHANNEL(T)(T x)
{
	return x & 0xFFF;
}

const auto CCV_ALL_DATA_TYPE = CCV_8U | CCV_32S | CCV_32F | CCV_64S | CCV_64F;
...
extern shared int _ccv_get_sparse_prime[];

int CCV_GET_SPARSE_PRIME(size_t x)
{
	return _ccv_get_sparse_prime[x];
}

alias void ccv_matrix_t;

alias void function(void*) ccv_cache_index_free_f;
 ...
 */
void* ccv_cache_get(ccv_cache_t* cache, ulong sign, ubyte* type);
/**
 ...
 */
int ccv_cache_put(ccv_cache_t* cache, ulong sign, void* x, uint size, ubyte type);
/**
 ...
 * @return The pointer to the object.
 */
void* ccv_cache_out(ccv_cache_t* cache, ulong sign, ubyte* type);
/**
 ...
 */
int ccv_cache_delete(ccv_cache_t* cache, ulong sign);
/**
 ...
 * @{
 */
ulong ccv_compute_dense_matrix_size(ulong rows, ulong cols, ulong type)
{
	return ccv_dense_matrix_t.sizeof +
		((cols * CCV_GET_DATA_TYPE_SIZE(type) * CCV_GET_CHANNEL(type) + 3) & -4) * rows;
}
 ...
 */
ccv_dense_matrix_t* ccv_dense_matrix_renew(ccv_dense_matrix_t* x, int rows, int cols, int types, int prefer_type, ulong sig);
/**
 ...
 */
ccv_dense_matrix_t* ccv_dense_matrix_new(int rows, int cols, int type, void* data, ulong sig);
/**
 ...
 */
ccv_dense_matrix_t ccv_dense_matrix(int rows, int cols, int type, void* data, ulong sig);
/**
 ...
 */
ccv_sparse_matrix_t* ccv_sparse_matrix_new(int rows, int cols, int type, int major, ulong sig);
/**
 ...
 */
ulong ccv_cache_generate_signature(const char* msg, int len, ulong sig_start, ...);

const int CCV_DEFAULT_CACHE_SIZE = (1024 * 1024 * 64);

/**
 * Drain up the cache.
 */
void ccv_drain_cache();
/**
...
/*
void* ccv_get_dense_matrix_cell(T)(T* x, row, col, ch)
{
	return ccv_get_dense_matrix_cell_by((x).type, x, row, col, ch);
}
*/
...
/*
void* ccv_get_dense_matrix_cell_value_by(ulong type, ccv_dense_matrix_t* x, ulong row, ulong col, ulong ch)
{
	return
...
/*
void* ccv_get_dense_matrix_cell_value(T)(T* x, ulong row, ulong col, ulong ch)
{
	return ccv_get_dense_matrix_cell_value_by((x).type, x, row, col, ch);
}
*/
 ...
 */
int ccv_read_impl(const void* inp, ccv_dense_matrix_t** x, int type, int rows, int cols, int scanline);

int ccv_read(const void* inp, ccv_dense_matrix_t** x, int type, int rows = 0, int cols = 0, int scanline = 0)
{
	return ccv_read_impl(inp, x, type, rows, cols, scanline);
}
 ...
 */
int ccv_write(ccv_dense_matrix_t* mat, char* outp, int* len, int type, void* conf);
/** @} */
...
ccv_margin_t ccv_margin(int left, int top, int right, int bottom)
{
	ccv_margin_t margin;
	margin.left = left;
	margin.top = top;
...
void ccv_float_to_half_precision(float* f, ushort* h, size_t len);
void ccv_half_precision_to_float(ushort* h, float* f, size_t len);
...
ccv_size_t ccv_size(int width, int height)
{
	ccv_size_t size;
	size.width = width;
	size.height = height;
...
int ccv_size_is_zero(ccv_size_t size)
{
...
int ccv_rect_is_zero(ccv_rect_t rect)
{
 ...
 */
ccv_array_t* ccv_array_new(int rsize, int rnum, ulong sig);
/**
...
alias int function(const void*, const void*, void*) ccv_array_group_f;
...
ccv_point_t ccv_point(int x, int y)
{
	ccv_point_t point;
	point.x = x;
	point.y = y;
...
ccv_decimal_point_t ccv_decimal_point(float x, float y)
{
...
struct ccv_decimal_pose_t{
	float x, y, a, b;
	float roll, pitch, yaw;
}

ccv_decimal_pose_t ccv_decimal_pose(float x, float y, float a, float b, float roll, float pitch, float yaw)
{
...
alias int function(const ccv_dense_matrix_t* x, double* f, ccv_dense_matrix_t* df, void*) ccv_minimize_f;
...
alias double function(double x, double y, void*) ccv_filter_kernel_f;
/**
...
const int CCV_DPM_PART_MAX = (10);

struct ccv_classification_t{
	int id;
	float confidence;
}
...
const int CCV_BBF_POINT_MAX = (8);
const int CCV_BBF_POINT_MIN = (3);
...
void ccv_ferns_feature(ccv_ferns_t* ferns, ccv_dense_matrix_t* a, int scale, uint* fern);
void ccv_ferns_correct(ccv_ferns_t* ferns, uint* fern, int c, int repeat);
float ccv_ferns_predict(ccv_ferns_t* ferns, uint* fern);
...
struct ccv_tld_t{
	...
	ulong frame_signature;
	int count;
	void* sfmt;
	void* dsfmt;
	uint fern_buffer[1]; // fetched ferns from image, this is a buffer
}
...
const int CCV_ICF_SAT_MAX = (2);

struct ccv_icf_feature_t{
	int count;
...
struct ccv_icf_decision_tree_t{
...
struct ccv_icf_classifier_cascade_t{
	int type;
	int count;
	int grayscale;
	ccv_margin_t margin;
...
struct ccv_icf_multiscale_classifier_cascade_t{
	int type;
	int count;
	int octave;
	int grayscale;
	ccv_icf_classifier_cascade_t* cascade;
} // Type B, scale the classifier

struct ccv_icf_param_t{
	int min_neighbors; /**< 0: no grouping afterwards. 1: group objects that intersects each other. > 1: group objects that intersects each other, and only passes these that have at least **min_neighbors** intersected objects. */
	int flags;
...
ccv_icf_multiscale_classifier_cascade_t* ccv_icf_multiscale_classifier_cascade_new(ccv_icf_classifier_cascade_t* cascades, int octave, int interval);
...
struct ccv_scd_stump_feature_t{
	int sx[4];
	int sy[4];
	int dx[4];
	int dy[4];
...
struct ccv_scd_stump_classifier_t{
	int count;
...
struct ccv_scd_decision_tree_t{
	int type;
	uint pass;
...
struct ccv_scd_classifier_cascade_t{
	int count;
	ccv_margin_t margin;
	ccv_size_t size;
...
ccv_categorized_t ccv_categorized(int c, ccv_dense_matrix_t* matrix, ccv_file_info_t* file)
{
...
union ccv_convnet_type_t{
	struct convolutional_t{
	...
	full_connect_t full_connect;
}

struct ccv_convnet_input_t{
	struct matrix_t{
...
struct ccv_convnet_layer_param_t{
...
struct ccv_convnet_layer_sgd_param_t{
...
struct ccv_convnet_layer_train_param_t{
...
struct ccv_convnet_train_param_t{
...
struct ccv_convnet_write_param_t{
